# z-util-page v3.2.1

## Cookie操作辅助类

- [CookieHelper](namespaces/CookieHelper/index.md)

## DOM操作辅助类

- [DomHelper](namespaces/DomHelper/index.md)

## HTTTP请求操作辅助类

- [Http](classes/Http.md)

## indexedDB操作辅助类

- [IDBHelper](classes/IDBHelper.md)

## 事件总线

- [EventBus](classes/EventBus.md)

## 响应式数据API

- [Reactive](namespaces/Reactive/index.md)

## 文件操作辅助类

- [FileHelper](namespaces/FileHelper/index.md)

## 辅助函数

- [debounce](functions/debounce.md)
- [deepClone](functions/deepClone.md)
- [generateUUID](functions/generateUUID.md)
- [getType](functions/getType.md)
- [mergeObject](functions/mergeObject.md)
- [parseUrl](functions/parseUrl.md)
- [throttle](functions/throttle.md)
