...
function warp(xhr, param, isInitHeader) {
...
var HttpHandle = {
    ...
    'multipart/form-data': function (xhr, param) {
        ...
        if (window.FormData) {
                    ...
                    if (type === "File" || type === "Blob") {
                        ...
                        readFile(file).load(function (res) {
                            ...
                            if (index_1 === 0) {
                                ...
                                Promise.resolve().then(function () {
                                    xhr.send(string2Uint8Array(combineResult_1));
                                });
                            }
                        }).start("BinaryString");
                    }
            ...
            if (index_1 === 0) {
                Promise.resolve().then(function () {
                    xhr.send(string2Uint8Array("--" + boundary_1 + "\r\n" + result_1.join("--" + boundary_1 + "\r\n") + "--" + boundary_1 + "--\r\n"));
                });
            }
        }
    }
};
function string2Uint8Array(value) {
    var nBytes = value.length, ui8Data = new Uint8Array(nBytes);
